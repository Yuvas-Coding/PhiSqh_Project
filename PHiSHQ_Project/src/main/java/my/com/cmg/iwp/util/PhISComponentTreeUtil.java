...
public class PhISComponentTreeUtil {
	...
	final private static class FieldListener implements IAddListener {
		...
		@Override
		public void addListener(Component component, StringBuilder result,
				int depth) {
			try {
	...
	static public CharSequence getZulTree(Component component) {
		return new PhISComponentTreeUtil().getZulTreeImpl(component);
	}

	private PhISComponentTreeUtil() {
		super();
	}
	...
	private StringBuilder getZulTreeImpl(Component component,
		...
		for (final Iterator<?> iterator = component.getChildren().iterator(); iterator
				.hasNext();) {
